...
export const idsFilePath = path.join(process.cwd(), 'data', 'sp-plugin', 'gossip-collectorids.json')
...
export async function writeIds({ articleIds, excludedIds }) {
  ...
  fs.writeFileSync(idsFilePath, JSON.stringify({ articleIds, excludedIds }, null, 2))
}
...
export async function fetchVideoById(videoId) {
  ...
  if (!articleIds.includes(videoId) || excludedIds.includes(videoId)) return null
  ...
  for (const base of videoUrls) {
    ...
    try {
      ...
      const data = await page.evaluate(() => {
        ...
        /* 视频 */
        document.querySelectorAll('.dplayer.dplayer-no-danmaku').forEach(dp => {
          try {
        ...
        document.querySelectorAll('img[src^="blob:"]').forEach(img => {
          if (!img.closest('.horizontal-banner, .article-bottom-apps')) r.images.push(img.src)
        })
        /* 正文 */
      ...
      if (data.videoUrls.length || data.articleContent.length || data.images.length) {
...
export async function searchArticlesByKeyword(keyword) {
  ...
  for (const base of bases) {
    try {
      const page = await browser.newPage()
      await page.goto(`${base}/search/${encodeURIComponent(keyword)}`, { waitUntil: 'networkidle2', timeout: 60000 })
      const arr = await page.evaluate(() => Array.from(document.querySelectorAll('article')).map(a => {
        const title = a.querySelector('h2.post-card-title')?.textContent?.trim()
...
export async function fetchPastArticles(count = 10) {
  ...
  for (const base of bases) {
    try {
      const page = await browser.newPage()
      await page.goto(`${base}/archives.html`, { waitUntil: 'networkidle2', timeout: 60000 })
      const arr = await page.evaluate(max => Array.from(document.querySelectorAll('.brick a')).slice(0, max).map(a => {
...
export async function refreshArticleIds() {
  ...
  for (const base of bases) {
    try {
      const page = await browser.newPage()
      await page.goto(`${base}/archives.html`, { waitUntil: 'domcontentloaded', timeout: 60000 })
      await page.waitForSelector('.brick a[href^="/archives/"]', { timeout: 30000 })
      const first = await page.evaluate(() => {
        const m = document.querySelector('.brick a[href^="/archives/"]')?.href?.match(/\/archives\/(\d+)/)
  ...
  if (maxId > 0) {
    const articleIds = Array.from({ length: maxId }, (_, i) => String(maxId - i))
    const { excludedIds } = await readIds()
    await writeIds({ articleIds, excludedIds })
    logger.info(`[gossip-utils] 已生成 1~${maxId} 共 ${articleIds.length} 个 ID`)
    return true
  }
  return false
}